...
class Pricelist_items extends CI_Controller {
	...
	public function list($id) {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			...
			$data['rows'] = $this->back_m->get_all_where($this->uri->segment(2), 'pricelist_id', $id);
			$data['pricelist'] = $this->back_m->get_one('pricelist', $id);
	...
	public function form($type, $pricelist_id, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
            // DEFAULT DATA
			$data = loadDefaultData();
            ...
            $data['pricelist'] = $this->back_m->get_one('pricelist', $pricelist_id);
	...
	public function action($type, $table, $pricelist_id, $id = '') {
		if(checkAccess($access_group = ['administrator', 'redaktor'], $_SESSION['rola'])) {
			$now = date('Y-m-d');
			...
			foreach ($_POST as $key => $value) {
				...
				if (!$this->db->field_exists('pricelist_id', $table)) {
					$this->base_m->create_num_column($table, 'pricelist_id');
				}
            ...
            if($type == 'insert') {
            	$insert['pricelist_id'] = $pricelist_id;
			...
			redirect('panel/'.$table. '/list/'. $pricelist_id);
		} else {
			redirect('panel');
		}
    }
